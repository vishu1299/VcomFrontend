...
export type DealFilterState = {
  ...
  /** Men's section: tshirts, shirts, jeans, trousers – when Shirts checked, show "Latest in Shirt" grid */
  selectedMensSubcategories: string[];
  brands: string[];
  ratings: number[];
  priceMin: number;
  priceMax: number;
...
export default function FilterSidebar({
  ...
  const toggleCategory = (id: string) => {
    ...
    const singleMain = next.length === 1 ? next[0] : null;
    const subcatsForNew = getSubcategoriesForMain(singleMain || 'Fashion & Apparel');
  ...
  const content = (
    <>
      ...
      <div className="flex flex-col gap-6 p-5">
        ...
        {filters.categories.includes('Fashion & Apparel') && filters.selectedSubcategories.includes('mens-wear') && (
          <section>
            <h4 className="text-base font-bold text-[#131313] mb-3">Men&apos;s</h4>
            <ul className="space-y-2">
              {MENS_SUBCATEGORIES.map((m) => (
                <li key={m.id} className="flex items-center justify-between gap-3">
                  <label htmlFor={`mens-sub-${m.id}`} className="flex-1 min-w-0 cursor-pointer text-sm text-[#131313]">
                    {m.label}
                  </label>
                  <input
                    id={`mens-sub-${m.id}`}
                    type="checkbox"
                    checked={filters.selectedMensSubcategories.includes(m.id)}
                    onChange={() => {
                      const next = filters.selectedMensSubcategories.includes(m.id)
                        ? filters.selectedMensSubcategories.filter((id) => id !== m.id)
                        : [...filters.selectedMensSubcategories, m.id];
                      onFiltersChange({ ...filters, selectedMensSubcategories: next });
                    }}
                    className="shrink-0 w-4 h-4 rounded cursor-pointer accent-[#1e3a8a]"
                  />
                </li>
