...
import { X, ChevronLeft, ChevronRight, ChevronDown, ChevronUp, Maximize2, Share2, Info, ShoppingCart, ArrowUpRight, Play } from 'lucide-react';
...
export default function ProductDetailModal({ product, isOpen, onClose }: ProductDetailModalProps) {
  ...
  const scrollThumbs = (dir: 'left' | 'right') => {
    ...
    const step = 120;
    thumbScrollRef.current.scrollBy({ left: dir === 'left' ? -step : step, behavior: 'smooth' });
  };
    ...
    >
      ...
      >
        ...
        <div className="absolute top-2 right-10 lg:top-4 lg:right-14 z-20 flex items-center gap-1.5 lg:gap-2">
          <button type="button" className="w-7 h-7 lg:w-9 lg:h-9 rounded-full bg-gray-200 border border-gray-400 flex items-center justify-center hover:bg-gray-300 transition" aria-label="Share">
            <Share2 className="w-3.5 h-3.5 lg:w-4 lg:h-4 text-gray-700" />
          </button>
          <button type="button" className="w-7 h-7 lg:w-9 lg:h-9 rounded-full bg-gray-200 border border-gray-400 flex items-center justify-center hover:bg-gray-300 transition" aria-label="Info">
                ...
                {videoUrl && activeMediaIndex === 0 && (
                  ...
                  >
                    <div className="w-14 h-14 lg:w-20 lg:h-20 rounded-full bg-white flex items-center justify-center shadow-md border border-black/10">
                      <Play className="w-6 h-6 lg:w-8 lg:h-8 text-[#1f2937] ml-0.5 lg:ml-1" fill="currentColor" />
                    </div>
                  </button>
                )}
              </>
            )}
            <button type="button" className="absolute top-2 right-2 lg:top-3 lg:right-3 w-7 h-7 lg:w-9 lg:h-9 rounded-lg bg-gray-200/90 hover:bg-gray-300 flex items-center justify-center text-gray-600" aria-label="Expand fullscreen">
              ...
              {thumbnails.map((t, i) => (
                <button
                  ...
                  className={`shrink-0 w-12 h-12 lg:w-16 lg:h-16 rounded-lg overflow-hidden border-2 bg-white transition ${
                    activeMediaIndex === i ? 'border-[#3F8CFF] ring-1 ring-[#3F8CFF]' : 'border-gray-200 hover:border-gray-300'
                  }`}
                >
                  <div className="relative w-full h-full">
                    <Image src={t} alt="" width={72} height={72} className="w-full h-full object-cover" unoptimized={t.startsWith('http')} />
                    {hasVideoOnThumb(i) && (
                      <div className="absolute bottom-0.5 left-0.5 lg:bottom-1 lg:left-1 w-4 h-4 lg:w-5 lg:h-5 rounded-full bg-white/90 flex items-center justify-center shadow">
                        <Play className="w-2 h-2 lg:w-2.5 lg:h-2.5 text-[#1f2937]" fill="currentColor" />
                      </div>
                    )}
                  </div>
                </button>
        ...
        <div className="lg:w-1/2 flex flex-col min-h-0 overflow-visible lg:overflow-y-auto">
          <div className="p-3 sm:p-4 lg:p-6 pt-3 lg:pt-4 pb-6 lg:pb-8 flex flex-col">
            <div className="flex flex-wrap gap-1.5 lg:gap-2 mb-1.5 lg:mb-2">
              {topBadges.map((b) => (
                <span key={b} className={`px-2 py-0.5 lg:px-2.5 lg:py-1 rounded text-[10px] lg:text-xs font-bold uppercase ${MODAL_BADGE_STYLES[b]}`}>
                  {BADGE_LABELS[b]}
                </span>
              ))}
            </div>
            <h2 className="text-sm lg:text-lg font-bold text-[#1f2937] leading-tight mb-2 lg:mb-3 truncate" title={`${name} | Polo`}>
              {name} | Polo
            </h2>
            <div className="text-xs lg:text-sm text-[#4b5563] space-y-0.5 mb-2 lg:mb-3">
              <p>SKU: <span className="font-semibold text-[#1f2937]">IP17-256-ORG</span></p>
              <p>Model Number: <span className="font-semibold text-[#1f2937]">A3102</span></p>
              <p>Category: <span className="font-semibold text-[#3F8CFF] underline cursor-pointer">Fashion</span></p>
            </div>

            <p className="text-xs lg:text-sm text-[#4b5563] mb-1 lg:mb-1.5">Select Size: <span className="font-semibold text-[#1f2937]">{selectedSize}</span></p>
            ...
            <button type="button" className="text-xs lg:text-sm text-[#3F8CFF] font-medium mb-2 lg:mb-3 flex items-center gap-1">
              Size Chart <ChevronDown className="w-3.5 h-3.5 lg:w-4 lg:h-4" />
            </button>

            <p className="text-xs lg:text-sm text-[#4b5563] mb-1 lg:mb-1.5">Select Color: <span className="font-semibold text-[#1f2937]">{COLORS[selectedColor].name}</span></p>
              ...
              {COLORS.map((c, i) => (
                <button
                  ...
                  className={`w-6 h-6 lg:w-8 lg:h-8 rounded-full border-2 transition shrink-0 ${
                    selectedColor === i ? 'border-gray-500 ring-2 ring-gray-400 ring-offset-1' : 'border-gray-300'
                  }`}
            ...
            <p className="text-xs lg:text-sm text-[#4b5563] mb-0.5">Model is 6&apos;0&quot;/185 cms and is wearing size M</p>
            <p className="text-xs lg:text-sm text-[#4b5563] mb-2 lg:mb-3">Cotton, Machine Wash.</p>

            <div className="flex items-baseline gap-2 lg:gap-3 mb-0.5">
              <span className="text-base lg:text-xl xl:text-2xl font-bold text-[#1f2937]">${typeof price === 'number' ? price.toFixed(2) : price}</span>
              {originalPrice != null && (
                <span className="text-xs lg:text-sm text-gray-400 line-through">${originalPrice.toFixed(2)}</span>
              )}
              {discount > 0 && (
                <span className="text-xs lg:text-sm font-semibold text-[#3F8CFF]">{discount}% OFF</span>
              )}
            </div>
            <p className="text-[10px] lg:text-xs text-gray-500 mb-3 lg:mb-4">Inclusive of all taxes</p>
            ...
            >
              Go to Product Page <ArrowUpRight className="w-3.5 h-3.5 lg:w-4 lg:h-4" />
            </Link>
            ...
            <div className="flex items-center border border-gray-300 rounded-lg overflow-hidden bg-white">
              ...
              <div className="flex flex-col border-l border-gray-300">
                <button type="button" onClick={() => setQuantity((q) => q + 1)} className="w-6 lg:w-8 h-4 lg:h-5 flex items-center justify-center text-gray-500 hover:bg-gray-100 border-b border-gray-300">
                  <ChevronUp className="w-3 h-3 lg:w-3.5 lg:h-3.5" />
                </button>
                <button type="button" onClick={() => setQuantity((q) => Math.max(1, q - 1))} className="w-6 lg:w-8 h-4 lg:h-5 flex items-center justify-center text-gray-500 hover:bg-gray-100">
