    ...
    <div className="relative z-10 w-full max-w-md rounded-xl bg-white shadow-lg border border-gray-200 overflow-visible">
      <div className="p-5 pt-4">
        <button
          type="button"
          onClick={onClose}
          className="absolute top-3 right-3 w-7 h-7 rounded-full flex items-center justify-center text-[#131313] hover:bg-gray-100 transition"
          aria-label="Close"
        >
          <svg
            width="18"
        ...
        <div className="flex items-center gap-2 rounded-lg border border-gray-200 bg-white overflow-hidden">
          <span className="pl-3 text-[#131313] shrink-0" aria-hidden>
            <svg
              width="16"
              height="16"
              viewBox="0 0 24 24"
...
function ReportSellerModal({
  ...
  const [reason, setReason] = useState(
    REPORT_REASONS[REPORT_REASONS.length - 1],
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4">
      ...
      <div className="relative w-full max-w-lg sm:max-w-2xl lg:max-w-5xl max-h-[95vh] sm:max-h-[90vh] rounded-xl sm:rounded-2xl bg-white shadow-xl p-3 sm:p-6 flex flex-col overflow-hidden">
        <button
          type="button"
          onClick={onClose}
          className="absolute top-2 right-2 sm:top-4 sm:right-4 w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-[#131313] hover:bg-gray-100 transition shrink-0"
          aria-label="Close"
        >
          <svg
            width="18"
        ...
        <h2 className="text-sm sm:text-base font-bold text-[#131313] pr-8">
          Report this seller
        </h2>
        <p className="text-xs sm:text-sm text-[#131313] mt-0.5 shrink-0">
          Your report helps keep our marketplace safe.
        </p>

        <div className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden">
          <p className="text-xs sm:text-sm font-bold text-[#131313] mt-2 sm:mt-3 mb-1.5">
            Select Reason
          </p>
          <div className="grid grid-cols-2 gap-1.5 sm:gap-2 mb-2 sm:mb-3">
            {REPORT_REASONS.map((r) => (
              <label
                key={r}
                className="flex items-center justify-between gap-2 p-1.5 sm:p-2 rounded-md border border-gray-200 cursor-pointer hover:bg-gray-50/50 transition"
              >
                <span className="text-[10px] sm:text-sm text-[#131313] line-clamp-2 sm:line-clamp-none leading-tight">
                  {r}
                </span>
                <input
                  type="radio"
          ...
          <div className="flex flex-col lg:flex-row gap-2 sm:gap-4 min-h-0 flex-1">
            <div className="min-w-0 flex-1 min-h-0">
              <p className="text-xs sm:text-sm font-bold text-[#131313] mb-1">
                Describe the issue
              </p>
              <textarea
                ...
                maxLength={MAX_CHARS}
                rows={3}
                className="w-full rounded-lg border border-gray-200 px-2.5 py-2 sm:px-3 sm:py-2.5 text-xs sm:text-sm text-[#131313] placeholder:text-[#131313] focus:ring-2 focus:ring-[#1E40AF] focus:border-[#1E40AF] focus:outline-none resize-none"
              />
              <p className="text-[10px] sm:text-xs text-[#131313] text-right mt-0.5">
                ({description.length}/{MAX_CHARS})
              </p>
            </div>
            <div className="min-w-0 flex-1 min-h-0 shrink-0">
              <p className="text-xs sm:text-sm font-bold text-[#131313] mb-1 sm:mb-2">
                Upload Supporting Media
              </p>
              <div className="border-2 border-dashed border-gray-200 rounded-lg p-3 sm:p-6 text-center">
                <div className="flex justify-center mb-1 sm:mb-2">
                  <span className="relative w-8 h-8 sm:w-12 sm:h-12 rounded-full bg-gray-100 flex items-center justify-center text-[#131313]">
                    <svg
                      width="18"
                      height="18"
                    ...
                    <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 sm:w-4 sm:h-4 bg-gray-400 rounded-full flex items-center justify-center text-white text-[10px] font-bold leading-none">
                      +
                    </span>
                  </span>
                </div>
                <p className="text-[10px] sm:text-sm text-[#131313] mb-0.5 sm:mb-1">
                  Choose file or drag & drop
                </p>
                <p className="text-[9px] sm:text-xs text-[#131313] mb-2 sm:mb-3">
                  JPEG, PNG, PDF, MP4 · 50 mb
                </p>
                <button
                  type="button"
                  className="text-[10px] sm:text-sm font-medium text-[#1E40AF] hover:underline"
                >
                  Browse File
                </button>
              </div>
        ...
        <div className="flex justify-end gap-2 sm:gap-3 mt-2 sm:mt-3 pt-2 shrink-0 border-t border-gray-100">
          <button
            type="button"
            onClick={onClose}
            className="px-3 py-2 sm:px-4 sm:py-2.5 rounded-lg border border-gray-200 bg-white text-[#131313] text-xs sm:text-sm font-medium hover:bg-gray-50 transition"
          >
            Cancel
          </button>
          <button
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                <div className="flex flex-wrap gap-2">
                  ...
                  <BadgeCheck
                    className="w-7 h-7 shrink-0 text-[#1d4ed8]"
                    aria-label="Verified"
                  />
                </div>
