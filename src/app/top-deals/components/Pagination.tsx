...
export default function Pagination({
  ...
  useEffect(() => {
    ...
    const update = () =>
      setMaxVisible(mql.matches ? MAX_VISIBLE_MOBILE : MAX_VISIBLE_DESKTOP);
    update();
  ...
  const prevNextClass = isDark
    ...
    : "min-h-[36px] sm:min-h-[40px] px-3 sm:px-4 rounded-lg border border-[#e5e7eb] bg-white text-gray-300 text-sm font-medium hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition flex items-center gap-2";
    ...
    isDark
      ...
      : `min-w-[32px] sm:min-w-[40px] h-9 sm:h-10 rounded-lg text-[13px] sm:text-sm font-medium transition shrink-0 border ${
          active
            ...
            : "bg-white border border-[#e5e7eb] text-gray-300 hover:bg-gray-50"
        }`;
