...
export type FilterState = ExclusiveFilterState;
...
export default function FilterSidebar({
  ...
  const toggleSubCategoryFilter = (id: string) => {
    const next = filters.subCategories.includes(id)
  ...
  const toggleTimeLeft = (id: string) => {
    const next = filters.timeLeft.includes(id)
      ? filters.timeLeft.filter((t) => t !== id)
      : [...filters.timeLeft, id];
    onFiltersChange({ ...filters, timeLeft: next });
  };

  const toggleTechSubCategory = (id: string) => {
    const next = filters.techSubCategories.includes(id)
      ? filters.techSubCategories.filter((c) => c !== id)
      : [...filters.techSubCategories, id];
    onFiltersChange({ ...filters, techSubCategories: next });
  };
      ...
      >
        ...
        >
          ...
          <section>
            ...
            <ul className="space-y-2">
              {BEAUTY_SUB_CATEGORY_FILTERS.map((c) => (
                <li key={c.id} className="flex items-center justify-between gap-3">
                  <label
                    htmlFor={`filter-subcat-${c.id}`}
                    className="flex-1 min-w-0 cursor-pointer flex items-baseline gap-1 truncate"
                  >
                    <span style={{ fontSize: FIGMA.valueFontSize, fontWeight: 400, color: FIGMA.black }}>{c.label}</span>
                    <span style={{ fontSize: FIGMA.valueFontSize, fontWeight: 400, color: FIGMA.muted }}> ({c.count})</span>
                  ...
                  <input
                    id={`filter-subcat-${c.id}`}
                    type="checkbox"
                    checked={filters.subCategories.includes(c.id)}
                    onChange={() => toggleSubCategoryFilter(c.id)}
                    className="shrink-0 rounded border cursor-pointer"
                    style={{
                      width: FIGMA.checkboxSize,
                      height: FIGMA.checkboxSize,
          ...
          <section>
            ...
            <ul className="space-y-2">
              {TIME_LEFT_OPTIONS.map((t) => (
                <li key={t.id} className="flex items-center justify-between gap-3">
                  <label
                    htmlFor={`filter-time-${t.id}`}
...
export { initialExclusiveFilters as initialFilters } from '../data/filters';
