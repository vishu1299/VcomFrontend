...
function HeartIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-4 h-4 text-[#131313]">
  ...
  return (
    <article className="w-full rounded-xl border border-[#e5e7eb] bg-white shadow-sm hover:shadow transition flex flex-col">
      <div className="relative aspect-square bg-white rounded-t-xl overflow-hidden">
        ...
        {discountBadge && (
          <span className="absolute top-2 left-2 bg-[#facc15] text-[#131313] text-[10px] font-semibold px-2 py-1 rounded-r-md">
            {discountBadge}
          </span>
        )}
        {newBadge && !sponsored && (
      ...
      <div className="px-3 pt-2 pb-3 flex flex-col flex-1">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 mt-auto">
          <div className="flex items-center gap-2">
            ...
            {originalPrice != null && (
              <span className="text-[1rem] text-gray-400 line-through">${originalPrice}</span>
            )}
          </div>
          <button
            type="button"
