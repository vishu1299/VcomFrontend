    ...
    >
      <div className="relative h-[200px] sm:h-[280px] md:h-[340px] lg:h-[400px]">
        ...
        <div className="absolute inset-0 flex flex-col justify-center px-4 sm:px-6 md:px-8 lg:px-12">
          <span className="text-design-14 sm:text-design-16 text-white/90 uppercase tracking-wide mb-1">
            {slide.tag}
          </span>
          ...
          <Link
            href="/product-list"
            className="btn-cta-sm w-fit mt-4 sm:mt-5 text-design-14 sm:text-design-16 bg-[var(--color-main-blue)]"
          >
            {slide.cta}
          </Link>
        </div>
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5 z-10">
        {[0, 1, 2, 3, 4].map((i) => (
          <button
            key={i}
            type="button"
            onClick={() => setActiveIndex(i % slides.length)}
